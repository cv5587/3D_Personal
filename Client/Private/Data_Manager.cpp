	...
	if (fin.is_open())
	{
		...
		for (_uint i = 0; i < Layersize; i++)
		{
			...
			for (_uint i = 0; i < iObjectSize; i++)
			{
				...
				{
					if (strPrototypeTag == TEXT("Prototype_GameObject_Portal"))
					{
					...
					else if (strPrototypeTag == TEXT("Prototype_GameObject_BoxProp"))
					{
						CBoxProp::BOXPROP_DESC BoxPropDesc{};
						BoxPropDesc.ProtoTypeTag = strPrototypeTag;
						BoxPropDesc.ModelTag = strModelTag;
						BoxPropDesc.vPrePosition = fWorldPosition;
						if (FAILED(m_pGameInstance->Add_CloneObject(iReadLevel, wLayer, strPrototypeTag, &BoxPropDesc)))
							return E_FAIL;
					}
					else if (strPrototypeTag == TEXT("Prototype_GameObject_Stove"))
					{
						CStove::STOVE_DESC StoveDesc{};
						StoveDesc.ProtoTypeTag = strPrototypeTag;
						StoveDesc.ModelTag = strModelTag;
						StoveDesc.vPrePosition = fWorldPosition;
						if (FAILED(m_pGameInstance->Add_CloneObject(iReadLevel, wLayer, strPrototypeTag, &StoveDesc)))
							return E_FAIL;
					}
				...
				else if (TEXT("Layer_Monster") == wLayer)
				{
					...
					if(TEXT("Prototype_Component_Model_Rabbit")== strModelTag)
					{
						CRabbit::RABBIT_DESC		RabbitDesc = {};

						strPrototypeTag= TEXT("Prototype_GameObject_Rabbit");
						RabbitDesc.ProtoTypeTag = strPrototypeTag;
						RabbitDesc.ModelTag = strModelTag;
						RabbitDesc.vPrePosition = fWorldPosition;
						RabbitDesc.CellIndex = CellIndex;
						RabbitDesc.isItem = false;
						if (FAILED(m_pGameInstance->Add_CloneObject(iReadLevel, wLayer, strPrototypeTag, &RabbitDesc)))
							return E_FAIL;
					}
					else if (TEXT("Prototype_Component_Model_Wolf") == strModelTag)
					{
						CWolf::WOLF_DESC		WolfDesc = {};

						strPrototypeTag = TEXT("Prototype_GameObject_Wolf");
						WolfDesc.ProtoTypeTag = strPrototypeTag;
						WolfDesc.ModelTag = strModelTag;
						WolfDesc.vPrePosition = fWorldPosition;
						WolfDesc.CellIndex = CellIndex;
						WolfDesc.isItem = false;
						//if (FAILED(m_pGameInstance->Add_CloneObject(iReadLevel, wLayer, strPrototypeTag, &WolfDesc)))
						//	return E_FAIL;
					}
				}
				else if(TEXT("Layer_Npc") == wLayer)
				{

					CMother::NPC_DESC		NPC_DESC ={};

					NPC_DESC.ProtoTypeTag = strPrototypeTag;
					NPC_DESC.ModelTag = strModelTag;
					NPC_DESC.vPrePosition = fWorldPosition;
		
					if (FAILED(m_pGameInstance->Add_CloneObject(iReadLevel, wLayer, strPrototypeTag, &NPC_DESC)))
						return E_FAIL;
				}
			}
...
HRESULT CData_Manager::Quest_Load(_uint iLevelIndex)
{
